	...
	int param;		/* integer parameter */
{
	...
	int begin = CurrentPlace;

	if (debug) Printf("Enter Spawn!\n");
	while(thread[CurrentPlace].valid == 1){
		CurrentPlace = (CurrentPlace + 1) % MAXTHREADS; 
		if(CurrentPlace == begin) return -1;
	}
...
void MySchedThread ()
{
	...
	if(queuesize == 1) MyYieldThread(front);
	else MyYieldThread (queue[front].next);
}
...
{
	if (! MyInitThreadsCalled) {
		if(debug) Printf ("ExitThread: Must call InitThreads first\n");
		Exit ();
	}
	int t = MyGetThread();
	thread[t].valid = 0;
